...
<?php else: ?> 
    <?php if(config_item('def_package') !== FALSE && config_item('def_package') == $listing->id &&  strtotime($user['package_last_payment'])<=time() && $listing->id !== $user['package_id']):?>
        <a href="<?php echo site_url('admin/packages/do_package_activate/'.$lang_code.'/'.config_item('def_package')); ?>" class="btn btn-middle btn-info"><?php echo lang_check('Activate');?></a>
    <?php endif; ?>                               
<?php endif; ?>       
                            </td>
                        </tr>
